...
FLAGS = {
    ...
    "إسبانيا":"🇪🇸","البرتغال":"🇵🇹","إنجلترا":"🏴󠁧󠁢󠁥الن󠁧󠁿","اسكتلندا":"🏴󠁧󠁢󠁳󠁣󠁴󠁿","المغرب":"🇲🇦",
...
# دالة ذكية وموحدة لاحتساب النقاط طبقاً للمبدأ الجديد
def calculate_match_points(p_h, p_a, p_p, actual_h, actual_a, actual_p, is_knockout):
    earned = 0
    # 1. حالة تطابق النتيجة الدقيقة
    if p_h == actual_h and p_a == actual_a:
        earned += 3
        if is_knockout and p_h == p_a and p_p == actual_p:
            earned += 3  # إضافة 3 نقاط إضافية لبطل الترجيح
    # 2. حالة عدم تطابق النتيجة الدقيقة ولكن صاب الفائز في الوقت الأصلي
    elif (p_h > p_a and actual_h > actual_a) or (p_h < p_a and actual_h < actual_a):
        earned += 1
    # 3. حالة توقع تعادل ليس بالنتيجة الدقيقة ولكن أصاب بطل الترجيح (في مباراة انتهت بالتعادل الفعلي)
...
if not st.session_state["is_logged_in"]:
    ...
    # --- تبويب لوحة الصدارة وعرض الإحصائيات الحية ---
    with tab_leaderboard:
        ...
        for idx, row in enumerate(leaderboard_data):
            ...
            with col_name: 
                if p_champ:
                    display_html = f"<div style='padding-top: 8px; font-size: 16px; font-weight: bold;'>{p_name} {FLAGS.get(p_champ, '🏳️')}</div>"
                else:
    ...
    # --- تبويب إدخال التوقعات والتعامل مع الترجيح ---
    with tab_predict:
        cursor = db_conn.cursor()
        ...
        for match in all_matches:
            ...
            if match_status_row and match_status_row[0] is not None and match_status_row[1] is not None:
                ...
                <div style='display:flex;justify-content:center;align-items:center;gap:6px;white-space:nowrap;overflow-x:auto;padding:0 5px;font-size:clamp(18px,4vw,24px);font-weight:bold;color:#FFD700;'>
                    <span>{away_flag} {match['team_away']}</span><span>{actual_a} × {actual_h}</span><span>{home_flag} {match['team_home']}</span> {txt_p}
                </div>
                ...
                with st.container():
                    ...
                    if (time_until_match < timedelta(minutes=10) or is_calculated_and_valid) and login_phone != ADMIN_PHONE:
                        ...
                        if match["is_knockout"] and h_score == a_score:
                            st.markdown("⚠️ **مباراة إقصائية وتوقعت التعادل! اختر الفائز بالترجيح لفرصة كسب 3 نقاط إضافية:**")
                            pens_winner = st.radio(f"الفائز بالترجيح لـ {match['team_home']} × {match['team_away']}", [match['team_home'], match['team_away']], index=0 if val_pens == match['team_home'] else 1, key=f"pens_{match['id']}", horizontal=True)
    ...
    if login_phone == ADMIN_PHONE:
        with tab_admin:
            ...
            if is_valid_old_result:
                ...
                st.warning(f"⚠️ هذه المباراة احتُسبت سابقاً بنتيجة: {already_calculated[0]} - {already_calculated[1]}{txt_p_old}")
            ...
            if selected_match["is_knockout"] and actual_h == actual_a:
                actual_p = st.radio("الفائز الفعلي بركلات الترجيح:", [selected_match['team_home'], selected_match['team_away']], index=0 if already_calculated and already_calculated[2] == selected_match['team_home'] else 1, key="admin_act_pens")
